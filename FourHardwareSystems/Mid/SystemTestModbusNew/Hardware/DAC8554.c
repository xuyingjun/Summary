...
void DAC7554_Send_16_bits(unsigned int data)  // data是一个24位的数据,STM32中int型是32位的数据
{
	char i=0;
...
void SetAoOut(float Parm,u8 channel,u8 CurrentFlag)
{
	...
	switch(CurrentFlag)
	{
		case 0:Dat=Parm/2.0/5.0*4096;break;
		case 1:
			     if(Parm<=3.98406)                              //3.98406=20*10/(10+40.2)
					 ...
					 {
						 Dat=(Parm-3.98406)/(20.08-3.98406)*10/2/5*4096;break;
					 }
		default:Dat=0;
	}
	switch(channel)
	{
		case 1:DAC7554_Send_16_bits((0x8000|0x0000)+Dat);break;
		case 2:DAC7554_Send_16_bits((0x8000|0x1000)+Dat);break;
		case 3:DAC7554_Send_16_bits((0x8000|0x2000)+Dat);break;
		case 4:DAC7554_Send_16_bits((0x8000|0x3000)+Dat);break;
		default:break;
	}
}	
/*****************************************************
...
******************************************************/
void Set8554Out(float Parm,u8 channel,u8 CurrentFlag)
{
	if(Parm<0)Parm=0;
	switch(CurrentFlag)
	{
