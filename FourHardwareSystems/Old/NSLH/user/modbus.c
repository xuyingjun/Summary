...
//	 Modbus_HoldReg[13]=(u16*)&PDVComm;
	 ...
	 Modbus_HoldReg[11]=(u16*)&IBSCommand;       //读取E-booking命令
	 Modbus_HoldReg[18]=(u16*)&ScheduleComm;     //读取Schedule命令
	 Modbus_HoldReg[19]=(u16*)&PDVComm;					 //StartStop命令,直接启停，不管有没Schedule和E-booking
	 
	 Modbus_HoldReg[20]=(u16*)&ValveCmd;							// 14, 阀紧急全开命令，不执行控制逻辑
	 Modbus_HoldReg[21]=(u16*)&ERVCmd;							  // 15; ERV紧急全开命令，不执行控制逻辑
	 Modbus_HoldReg[22]=(u16*)&ERVState;							// 16; ERV状态反馈，0为关，1为全开，其余数值为开度2-100

	 Modbus_HoldReg[23]=(u16*)&PurgeCmd;							// 17; Purge开关命令，0为关，1为全开；开持续10分钟
	 Modbus_HoldReg[24]=(u16*)&TempSPSet_Unocc;				// 18; 无人模式下温度设定值	 
	 Modbus_HoldReg[25]=(u16*)&SpareSet1;							// 19; 备用参数设置1，0-100
	 Modbus_HoldReg[32]=(u16*)&SpareSet2;							// 20; 备用参数设置2，0-100
	 Modbus_HoldReg[33]=(u16*)&SpareSet3;							// 21; 备用参数设置3，0-100
}
